...
using fmha_epilogue =
    ck_tile::DefaultGemm2DEpilogue<ck_tile::DefaultGemm2DEpilogueProblem<
                               typename FmhaFwdTypeConfig<fmha_dtype>::PDataType,
                               typename FmhaFwdTypeConfig<fmha_dtype>::VDataType,
                               ck_tile::tuple<>,
                               typename FmhaFwdTypeConfig<fmha_dtype>::OaccDataType,
                               ...
                               fmha_shape::kN1,
                               {F_spad}, {F_dvpad},
                               fmha_shape::Gemm1WarpTile::at(ck_tile::number<0>{{}}),
                               fmha_shape::Gemm1WarpTile::at(ck_tile::number<1>{{}}),
                               fmha_shape::Gemm1WarpTile::at(ck_tile::number<2>{{}}),
                               true>>;
...
class FmhaFwdApiTrait:
    ...
    def seqtune(self, max_bm0: int) -> str:
        if str(self.hdim) == "256":
            if self.bm0 == max_bm0 and max_bm0 <= 32:
                return "true/*D256 fall back tile without bm64*/"
            if self.bm0 == 16:
                return "a.seqlen_q <= 64"
            if self.bm0 == 32:
                return "a.seqlen_q <= 128"
            if self.bm0 == 64 and self.bn0 == 64:
                return "a.seqlen_q <= 1024"
            if self.bm0 == max_bm0:
                return "true/*D256 fall back tile*/"
        d128_bm64_short_cutoff = (
            0 if os.getenv("CK_TILE_DISABLE_FWD_D128_BM64_SHORT_S512", "0") == "1" else 512
        )
        if os.getenv("CK_TILE_DEBUG_FWD_D128_BM64_SHORT_S512", "0") == "1":
            d128_bm64_short_cutoff = 512
        elif os.getenv("CK_TILE_DEBUG_FWD_D128_BM64_SHORT_S256", "0") == "1":
            d128_bm64_short_cutoff = 256
        if str(self.hdim) == "128" and d128_bm64_short_cutoff:
            if self.bm0 == 64:
                return f"a.seqlen_q <= {d128_bm64_short_cutoff}"
            if self.bm0 == max_bm0:
                return "true/*D128 short tile trial fallback*/"
        if self.bm0 == max_bm0:
...
class KernelComponentFactoryGfx11(CompatibilityRuleFactory):
    ...
    def get_hdim_tile_size_dict(cls, dtype: str) -> Optional[dict]:
        if dtype in cls._DT_FP16_BF16:
            tiles = {
                #                             bm0, bn0, bk0, bn1, bk1, hdim, rm0, rn0, rk0, rm1, rn1, rk1, wm0, wn0, wk0, wm1, wn1, wk1, grp
                # HDIM 32: 极速方案 (256线程/8Waves)，利用高占有率掩盖访存
                ( 32,  32) : [FmhaFwdTileSize( 32,  32,  16,  32,  16,   32,  2, 1, 1,  2, 1, 1,  16, 16, 16,  16, 16, 16,  -1)],

                # HDIM 64: 黄金平衡 (128线程)，实测 176ms 纪录保持者
                ( 64,  64) : [
                    # Config 1: 极速冠军 (128线程轻量化) - 适用于长序列
                    FmhaFwdTileSize( 64,  32,  32,  64,  32,   64,  4, 1, 1,  4, 1, 1,  16, 16, 16,  16, 16, 16,  -1),
                    # Config 2: 稳健方案 (256线程) - 适用于短序列高吞吐
                    FmhaFwdTileSize( 64,  64,  32,  64,  32,   64,  4, 1, 1,  4, 1, 1,  16, 16, 16,  16, 16, 16,  -1),
                ],

                # HDIM 128: 核心优化目标 (Llama 专用)
                (128, 128) : [
                    # Config 1: 绝对巅峰 (M64_N32_128线程) - 极速纯寄存器流
                    FmhaFwdTileSize( 64,  32,  16, 128,  32,  128,  4, 1, 1,  4, 1, 1,  16, 16, 16,  16, 16, 16,  -1),

                    # Config 2: 长序列霸主 (M128_N32_256线程) - 极致算术强度
                    FmhaFwdTileSize(128,  32,  16, 128,  16,  128,  8, 1, 1,  8, 1, 1,  16, 16, 16,  16, 16, 16,  -1),
                ],

                # HDIM 192: 继承 HDIM 128 的长方形分块逻辑
                (192, 128) : [FmhaFwdTileSize( 64,  32,  32, 192,  32,  256,  4, 1, 1,  4, 1, 1,  16, 16, 16,  16, 16, 16,  -1)],

                # HDIM 256: keep the conservative tile as fallback; bm64 is enabled below
                # by default on gfx11 after D256 BF16 S1024/S2048 validation.
                (256, 256) : [FmhaFwdTileSize( 32,  32,  32, 256,  32,  256,  2, 1, 1,  2, 1, 1,  16, 16, 16,  16, 16, 16,  -1)],
            }  # fmt: skip
            d128_tile_gates = (
                os.getenv("CK_TILE_DEBUG_FWD_D128_BM64_BN64", "0") == "1",
                os.getenv("CK_TILE_DEBUG_FWD_D128_FORCE_BM128", "0") == "1",
            )
            d256_tile_gates = (
                os.getenv("CK_TILE_DEBUG_FWD_D256_SHORT_BM16", "0") == "1",
                os.getenv("CK_TILE_DEBUG_FWD_D256_BM64_BN32", "0") == "1",
                os.getenv("CK_TILE_DEBUG_FWD_D256_BM64_BN128", "0") == "1",
            )
            enable_d256_bm64 = os.getenv("CK_TILE_DISABLE_FWD_D256_BM64", "0") != "1"
            enable_d256_bm16 = os.getenv("CK_TILE_DISABLE_FWD_D256_BM16", "0") != "1"
            if sum(d128_tile_gates) > 1:
                raise ValueError("FWD D128 debug tile gates are mutually exclusive")
            if sum(d256_tile_gates) > 1:
                raise ValueError("FWD D256 debug tile gates are mutually exclusive")

            # D256 debug gates override the default D256 short/long tile policy.
            if enable_d256_bm16 and not any(d256_tile_gates):
                tiles[(256, 256)].insert(
                    0,
                    FmhaFwdTileSize( 16,  32,  32, 256,  32,  256,  1, 1, 1,  1, 1, 1,  16, 16, 16,  16, 16, 16,  -1),
                )  # fmt: skip
            if enable_d256_bm64 and not any(d256_tile_gates):
                tiles[(256, 256)].extend([
                    FmhaFwdTileSize( 64,  64,  32, 256,  32,  256,  4, 1, 1,  4, 1, 1,  16, 16, 16,  16, 16, 16,  -1),
                    FmhaFwdTileSize( 64,  32,  32, 256,  32,  256,  4, 1, 1,  4, 1, 1,  16, 16, 16,  16, 16, 16,  -1),
                ])  # fmt: skip
            if os.getenv("CK_TILE_DEBUG_FWD_D64_BN64_FIRST", "0") == "1":
                tiles[(64, 64)] = [
                    FmhaFwdTileSize( 64,  64,  32,  64,  32,   64,  4, 1, 1,  4, 1, 1,  16, 16, 16,  16, 16, 16,  -1),
                    FmhaFwdTileSize( 64,  32,  32,  64,  32,   64,  4, 1, 1,  4, 1, 1,  16, 16, 16,  16, 16, 16,  -1),
                ]  # fmt: skip
            if d128_tile_gates[0]:
                tiles[(128, 128)] = [
                    FmhaFwdTileSize( 64,  64,  32, 128,  32,  128,  4, 1, 1,  4, 1, 1,  16, 16, 16,  16, 16, 16,  -1),
                    FmhaFwdTileSize(128,  32,  16, 128,  16,  128,  8, 1, 1,  8, 1, 1,  16, 16, 16,  16, 16, 16,  -1),
                ]  # fmt: skip
            if d128_tile_gates[1]:
                tiles[(128, 128)] = [
                    FmhaFwdTileSize(128,  32,  16, 128,  16,  128,  8, 1, 1,  8, 1, 1,  16, 16, 16,  16, 16, 16,  -1),
                ]  # fmt: skip
            if d256_tile_gates[0]:
                tiles[(256, 256)] = [
                    FmhaFwdTileSize( 16,  32,  32, 256,  32,  256,  1, 1, 1,  1, 1, 1,  16, 16, 16,  16, 16, 16,  -1),
                    FmhaFwdTileSize( 32,  32,  32, 256,  32,  256,  2, 1, 1,  2, 1, 1,  16, 16, 16,  16, 16, 16,  -1),
                    FmhaFwdTileSize( 64,  64,  32, 256,  32,  256,  4, 1, 1,  4, 1, 1,  16, 16, 16,  16, 16, 16,  -1),
                    FmhaFwdTileSize( 64,  32,  32, 256,  32,  256,  4, 1, 1,  4, 1, 1,  16, 16, 16,  16, 16, 16,  -1),
                ]  # fmt: skip
            if d256_tile_gates[1]:
                tiles[(256, 256)] = [
                    FmhaFwdTileSize( 32,  32,  32, 256,  32,  256,  2, 1, 1,  2, 1, 1,  16, 16, 16,  16, 16, 16,  -1),
                    FmhaFwdTileSize( 64,  32,  32, 256,  32,  256,  4, 1, 1,  4, 1, 1,  16, 16, 16,  16, 16, 16,  -1),
                ]  # fmt: skip
            if d256_tile_gates[2]:
                tiles[(256, 256)] = [
                    FmhaFwdTileSize( 32,  32,  32, 256,  32,  256,  2, 1, 1,  2, 1, 1,  16, 16, 16,  16, 16, 16,  -1),
                    FmhaFwdTileSize( 64, 128,  32, 256,  32,  256,  4, 1, 1,  4, 1, 1,  16, 16, 16,  16, 16, 16,  -1),
                ]  # fmt: skip
            return tiles
            ...
            return {
                ...
                (128, 128) : [
                    ...
                    FmhaFwdTileSize( 96,  32,  32, 128,  32,  128,  6, 1, 1,  6, 1, 1,  16, 16, 16,  16, 16, 16,  -1),
                    FmhaFwdTileSize(128,  32,  32, 128,  32,  128,  8, 1, 1,  8, 1, 1,  16, 16, 16,  16, 16, 16,  -1),
                ],
                (256, 256) : [FmhaFwdTileSize( 32,  32,  32, 256,  32,  256,  2, 1, 1,  2, 1, 1,  16, 16, 16,  16, 16, 16,  -1)],
            }  # fmt: skip
        elif dtype in cls._DT_FP8FP32:
            return {
                #                             bm0, bn0, bk0, bn1, bk1,
                (128, 128) : [
                    # Config 1: Survival - very long sequences
                    FmhaFwdTileSize( 32,  32,  32, 128,  32,  128,  2, 2, 1,  2, 2, 1,  16, 16, 16,  16, 16, 16,  -1),
                    # Config 2: Balanced - medium sequences
                    FmhaFwdTileSize( 64,  64,  32, 128,  32,  128,  4, 1, 1,  4, 1, 1,  16, 16, 16,  16, 16, 16,  -1),
                    # Config 3: Optimized - long sequences
                    FmhaFwdTileSize( 64,  32,  32, 128,  32,  128,  4, 1, 1,  4, 1, 1,  16, 16, 16,  16, 16, 16,  -1),
                    # Config 4: Heavy assault - short sequences
                    FmhaFwdTileSize(128,  64,  32, 128,  32,  128,  4, 1, 1,  4, 1, 1,  16, 16, 16,  16, 16, 16,  -1),
                ],
            }  # fmt: skip
        else:
    ...
    def get_hdim_tile_size_dict(cls, dtype: str) -> Optional[dict]:
        if dtype in cls._DT_FP16_BF16:
            return {
                ...
                ( 32,  32) : [FmhaFwdTileSize( 64,  64,  16,  32,  32,   32,  4, 1, 1,  4, 1, 1,  16, 16, 16,  16, 16, 16,  -1)],
                ( 64,  64) : [
                    FmhaFwdTileSize( 32,  32,  32,  64,  32,   64,  2, 2, 1,  2, 2, 1,  16, 16, 16,  16, 16, 16,  -1),
                    FmhaFwdTileSize( 64,  64,  32,  64,  32,   64,  4, 1, 1,  4, 1, 1,  16, 16, 16,  16, 16, 16,  -1),
                    FmhaFwdTileSize( 64,  32,  32,  64,  32,   64,  4, 1, 1,  4, 1, 1,  16, 16, 16,  16, 16, 16,  -1),
                    FmhaFwdTileSize(128,  64,  32,  64,  32,   64,  4, 1, 1,  4, 1, 1,  16, 16, 16,  16, 16, 16,  -1),
                ],
                (128, 128) : [
                    FmhaFwdTileSize( 32,  32,  32, 128,  32,  128,  2, 2, 1,  2, 2, 1,  16, 16, 16,  16, 16, 16,  -1),
                    FmhaFwdTileSize( 64,  64,  32, 128,  32,  128,  4, 1, 1,  4, 1, 1,  16, 16, 16,  16, 16, 16,  -1),
