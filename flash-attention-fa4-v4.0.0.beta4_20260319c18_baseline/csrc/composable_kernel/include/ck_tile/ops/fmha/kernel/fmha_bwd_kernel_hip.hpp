...
#include "ck_tile/ops/fmha/pipeline/block_fmha_bwd_dq_qmajor_pipeline_kr_ktr_vr_iglp_hip.hpp"
...
struct fmha_bwd_pipeline_has_kDoDK : std::false_type
{
};

template <typename T>
struct fmha_bwd_pipeline_has_kDoDK<T, std::void_t<decltype(T::kDoDK)>> : std::true_type
{
};

template <typename T, bool = fmha_bwd_pipeline_has_kDoDK<T>::value>
struct fmha_bwd_pipeline_kDoDK : std::true_type
{
};

template <typename T>
struct fmha_bwd_pipeline_kDoDK<T, true> : std::bool_constant<T::kDoDK>
{
};

template <typename T, typename = void>
struct fmha_bwd_pipeline_has_kDoDV : std::false_type
{
};

template <typename T>
struct fmha_bwd_pipeline_has_kDoDV<T, std::void_t<decltype(T::kDoDV)>> : std::true_type
{
};

template <typename T, bool = fmha_bwd_pipeline_has_kDoDV<T>::value>
struct fmha_bwd_pipeline_kDoDV : std::true_type
{
};

template <typename T>
struct fmha_bwd_pipeline_kDoDV<T, true> : std::bool_constant<T::kDoDV>
{
};
} // namespace detail
...
struct FmhaBwdDQDKDVKernel
{
    ...
    static constexpr bool kDoDQ            = FmhaPipeline::kDoDQ;
    static constexpr bool kDoDKDV          = FmhaPipeline::kDoDKDV;
    static constexpr bool kDoDK            = detail::fmha_bwd_pipeline_kDoDK<FmhaPipeline>::value;
    static constexpr bool kDoDV            = detail::fmha_bwd_pipeline_kDoDV<FmhaPipeline>::value;
    ...
    CK_TILE_HOST static std::string GetName()
    {
        ...
        return
            ...
            (kDoDQ && kDoDKDV ? "" :
             (kDoDKDV ? (kDoDK && kDoDV ? "_split_dkdv" : (kDoDK ? "_split_dk" : "_split_dv"))
                      : "_split_dq"));
        #undef _SS_
        #undef _TS_
        // clang-format on
    ...
    CK_TILE_DEVICE void run_(Kargs kargs) const
    {
        ...
        if constexpr(!kUseQrQtrDorPipeline)
        {
            ...
            if constexpr(kDoDK && kPadHeadDimQ > 0)
            {
                impl::insert_dummy_dep(dk_acc_tile.get_thread_buffer());
            }
            if constexpr(kDoDV && kPadHeadDimV > 0)
            {
...
template <typename FmhaPipeline_,
          typename KGradEpiloguePipeline_,
          typename VGradEpiloguePipeline_,
          typename QGradEpiloguePipeline_>
struct FmhaBwdDQQMajorKernel
{
    using FmhaPipeline          = ck_tile::remove_cvref_t<FmhaPipeline_>;
    using QGradEpiloguePipeline = ck_tile::remove_cvref_t<QGradEpiloguePipeline_>;
    using BaseKernel =
        FmhaBwdDQDKDVKernel<FmhaPipeline_,
                             KGradEpiloguePipeline_,
                             VGradEpiloguePipeline_,
                             QGradEpiloguePipeline_>;

    using Kargs = typename BaseKernel::Kargs;
    ...
    static_assert(ck_tile::fmha_bwd_dq_qmajor_pipeline<FmhaPipeline>::value,
                  "FmhaBwdDQQMajorKernel requires a Q-major DQ pipeline.");
    static_assert(!kIsGroupMode, "Q-major split DQ trial supports batch mode only.");

    template <typename... Ts>
    CK_TILE_HOST static constexpr Kargs MakeKargsImpl(Ts... args)
    {
        return BaseKernel::MakeKargsImpl(args...);
    }

    CK_TILE_HOST static std::string GetName() { return BaseKernel::GetName() + "_qmajor"; }
    ...
    CK_TILE_HOST static dim3 BlockSize() { return BaseKernel::BlockSize(); }
    ...
    CK_TILE_DEVICE void run_(Kargs kargs) const
    {
        ...
        if(kargs.seqlen_q <= i_m0 || kargs.seqlen_q != kargs.seqlen_k ||
           kargs.nhead_ratio_qk != 1)
        {
            return;
        }
        ...
        const OGradDataType* do_ptr =
            reinterpret_cast<const OGradDataType*>(kargs.do_ptr) +
            static_cast<long_index_t>(i_nhead) * kargs.nhead_stride_do + batch_offset_do;
        ...
        const DDataType* d_ptr = reinterpret_cast<const DDataType*>(kargs.d_ptr) +
                                 static_cast<long_index_t>(i_nhead) * kargs.nhead_stride_lsed +
                                 batch_offset_lsed;
        QGradDataType* dq_ptr = reinterpret_cast<QGradDataType*>(kargs.dq_acc_ptr) +
        ...
        constexpr auto mn_lengths =
            make_tuple(number<FmhaPipeline::kM0>{}, number<FmhaPipeline::kN0>{});
        auto bias_dram_window    = make_null_tile_window(mn_lengths);
        auto randval_dram_window = make_null_tile_window(mn_lengths);
        auto dbias_dram_window   = make_null_tile_window(mn_lengths);
        ...
        FmhaPipeline{}(smem_ptr,
                       ...
                       QGradEpiloguePipeline{},
                       mask,
                       EmptyPositionEncoding<typename FmhaPipeline::AccDataType>{},
                       kargs.raw_scale,
                       kargs.scale,
                       1.0f,
                       1.0f,
