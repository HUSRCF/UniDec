...
#ifndef CK_TILE_DEBUG_O_GLB_AFTER
#define CK_TILE_DEBUG_O_GLB_AFTER 0
#endif

#ifndef CK_TILE_DEBUG_O_GLB_B
#define CK_TILE_DEBUG_O_GLB_B 0
#endif

#ifndef CK_TILE_DEBUG_O_GLB_H
#define CK_TILE_DEBUG_O_GLB_H 0
#endif

#ifndef CK_TILE_DEBUG_O_GLB_M
#define CK_TILE_DEBUG_O_GLB_M 0
#endif

#ifndef CK_TILE_DEBUG_O_GLB_N
#define CK_TILE_DEBUG_O_GLB_N 0
#endif
...
struct FmhaFwdKernel
{
    ...
    CK_TILE_DEVICE void run_(Kargs kargs) const
    {
        if constexpr(kPipelineName != "qr_async_trload")
        {
            ...
            if(blockIdx.x < 8 && blockIdx.y == 0 && blockIdx.z < 2 && threadIdx.x == 0)
            {
                printf("[O_PTR] b=(%d,%d,%d) mode=%d i_m0=%d i_n1=%d i_nhead=%d i_batch=%d "
                       "o_base=%p o_ptr=%p off_o=%lld stride_o=%lld nhead_stride_o=%lld "
                       "seqlen_q=%d hdim_v=%d\n",
                       static_cast<int>(blockIdx.x),
                       static_cast<int>(blockIdx.y),
                       static_cast<int>(blockIdx.z),
                       static_cast<int>(kIsGroupMode),
                       static_cast<int>(i_m0),
                       static_cast<int>(i_n1),
                       static_cast<int>(i_nhead),
                       static_cast<int>(i_batch),
                       kargs.o_ptr,
                       o_ptr,
                       static_cast<long long>(batch_offset_o),
                       static_cast<long long>(kargs.stride_o),
                       static_cast<long long>(kargs.nhead_stride_o),
                       static_cast<int>(kargs.seqlen_q),
                       static_cast<int>(kargs.hdim_v));
            }
#endif
            ...
            if(blockIdx.x < 8 && blockIdx.y == 0 && blockIdx.z < 2 && threadIdx.x == 0)
            {
                printf("[O_WIN] b=(%d,%d,%d) mode=%d i_m0=%d i_n1=%d m0=%d n1=%d "
                       "seqlen_q=%d hdim_v=%d\n",
                       static_cast<int>(blockIdx.x),
                       static_cast<int>(blockIdx.y),
                       static_cast<int>(blockIdx.z),
                       ...
                       static_cast<int>(i_m0),
                       static_cast<int>(i_n1),
                       static_cast<int>(FmhaPipeline::kM0),
                       static_cast<int>(FmhaPipeline::kN1),
                       static_cast<int>(kargs.seqlen_q),
                       static_cast<int>(kargs.hdim_v));
            }
#endif
...
#if CK_TILE_DEBUG_O_GLB_AFTER && defined(__HIP_DEVICE_COMPILE__)
            if(blockIdx.x < 8 && blockIdx.y == 0 && blockIdx.z < 2 && threadIdx.x == 0)
            {
                constexpr index_t kDbgB = CK_TILE_DEBUG_O_GLB_B;
                constexpr index_t kDbgH = CK_TILE_DEBUG_O_GLB_H;
                constexpr index_t kDbgM = CK_TILE_DEBUG_O_GLB_M;
                constexpr index_t kDbgN = CK_TILE_DEBUG_O_GLB_N;

                const bool match_bh = (i_batch == kDbgB) && (i_nhead == kDbgH);
                const bool in_tile =
                    (kDbgM >= i_m0) && (kDbgM < (i_m0 + FmhaPipeline::kM0)) &&
                    (kDbgN >= i_n1) && (kDbgN < (i_n1 + FmhaPipeline::kN1));

                if(match_bh && in_tile)
                {
                    const auto o_idx        = make_tuple(kDbgM, kDbgN);
                    const index_t o_abs_off = o_dram.get_tensor_descriptor().calculate_offset(o_idx);
                    using o_scalar_t =
                        remove_cvref_t<decltype(o_dram.get_buffer_view()(o_abs_off))>;
                    __threadfence_system();
                    const volatile o_scalar_t* o_probe_ptr =
                        reinterpret_cast<volatile o_scalar_t*>(&o_dram.get_buffer_view()(o_abs_off));
                    const float o_after_value =
                        type_convert<float>(static_cast<o_scalar_t>(*o_probe_ptr));

                    printf("[O_GLB_AFTER] b=(%d,%d,%d) mode=%d q=(%d,%d,%d,%d) tile=(%d,%d) off=%lld v=%f\\n",
                           static_cast<int>(blockIdx.x),
                           static_cast<int>(blockIdx.y),
                           static_cast<int>(blockIdx.z),
                           static_cast<int>(kIsGroupMode),
                           static_cast<int>(kDbgB),
                           static_cast<int>(kDbgH),
                           static_cast<int>(kDbgM),
                           static_cast<int>(kDbgN),
                           static_cast<int>(i_m0),
                           static_cast<int>(i_n1),
                           static_cast<long long>(o_abs_off),
                           o_after_value);

                    if(kDbgN < kargs.seqlen_q && kDbgM < kargs.hdim_v)
                    {
                        const auto o_swap_idx        = make_tuple(kDbgN, kDbgM);
                        const index_t o_swap_abs_off =
                            o_dram.get_tensor_descriptor().calculate_offset(o_swap_idx);
                        const volatile o_scalar_t* o_swap_probe_ptr =
                            reinterpret_cast<volatile o_scalar_t*>(
                                &o_dram.get_buffer_view()(o_swap_abs_off));
                        const float o_swap_value =
                            type_convert<float>(static_cast<o_scalar_t>(*o_swap_probe_ptr));

                        printf("[O_GLB_SWAP] b=(%d,%d,%d) mode=%d q=(%d,%d,%d,%d) swap=(%d,%d) off=%lld v=%f\\n",
                               static_cast<int>(blockIdx.x),
                               static_cast<int>(blockIdx.y),
                               static_cast<int>(blockIdx.z),
                               static_cast<int>(kIsGroupMode),
                               ...
                               static_cast<int>(kDbgN),
                               static_cast<int>(kDbgM),
                               static_cast<long long>(o_swap_abs_off),
                               o_swap_value);
                    }
                }
            }
