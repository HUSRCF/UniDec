...
#ifndef CK_TILE_DEBUG_BWD_SPLIT_DQ_TAIL_DIRECT_UPDATE
#define CK_TILE_DEBUG_BWD_SPLIT_DQ_TAIL_DIRECT_UPDATE 0
#endif
...
struct BlockFmhaBwdDQDKDVPipelineKRKTRVRIGLP
{
    ...
    static constexpr bool kDoDQ            = kDoDQ_;
    static constexpr bool kDoDKDV          = kDoDKDV_;
    static constexpr bool kIsFusedFullBwd  = kDoDQ && kDoDKDV;
    static_assert(!kUseTrLoad, "This pipeline does not use trload!");
    static_assert(kDoDQ || kDoDKDV, "At least one BWD output path must remain enabled.");
    ...
    CK_TILE_HOST_DEVICE static constexpr ck_tile::index_t GetSmemSize()
    {
        ...
        constexpr index_t kQTGuardSmemSize = Policy::template GetSmemSizeQT<Problem>();
        constexpr index_t kDQRemapSmemSize =
            kDoDQ ? kM0 * kQKHeaddim * sizeof(AccDataType) : 0;
        constexpr index_t kHardSplitSmemSize = kBaseSmemSize + kQTGuardSmemSize;
        return kHardSplitSmemSize > kDQRemapSmemSize ? kHardSplitSmemSize : kDQRemapSmemSize;
    }
               ...
               FmhaDropout& dropout) const
    {
        ...
        constexpr index_t kQTGuardSize = Policy::template GetSmemSizeQT<Problem>();
        constexpr index_t kQTBaseSize = Policy::template GetSmemSizeQT<Problem>() + kQTGuardSize;

        QDataType* q_lds_ptr = static_cast<QDataType*>(static_cast<void*>(
            static_cast<char*>(smem_ptr) + kQTBaseSize + Policy::template GetSmemSizeOGrad<Problem>() +
        ...
        QDataType* qt_lds_ptr = static_cast<QDataType*>(static_cast<void*>(smem_ptr));
        ...
        OGradDataType* do_lds_ptr =
            static_cast<OGradDataType*>(static_cast<void*>(static_cast<char*>(smem_ptr) + kQTBaseSize));
        ...
        auto qt_reg_tensor_tail = [&]() {
            using QTRegTensorType = decltype(load_tile(qt_lds_read_window));
        ...
        while(i_total_loops < (num_total_loop - 1))
        {
            ...
            if constexpr(kIsFusedFullBwd)
            {
                ...
                Policy::template PTFromGemm0CToGemm1A<Problem>(pt_reg_tensor, p_gemm, ds_lds_ptr);
                gemm_1(dv_acc, pt_reg_tensor, dot_reg_tensor);
            }
            else if constexpr(kDoDKDV)
            {
                const auto p_gemm = [&]() {
                    if constexpr(FmhaDropout::IsDropout)
                    {
            ...
            if constexpr(!kIsFusedFullBwd)
            {
                if(i_total_loops == (num_total_loop - 2))
                {
                    block_sync_lds();
                    if constexpr(kDoDKDV)
                    {
                        auto qt_lds_last_rd = load_tile(qt_lds_read_window);
                        qt_reg_tensor_tail  = qt_lds_last_rd;
                    }
                    block_sync_lds();
                }
            }
        ...
        {
            Policy::template SGradTFromGemm2CToGemm3A<Problem>(dst_reg_tensor,
                                                               ds_gemm,
                                                               ds_lds_ptr);
            gemm_3(dk_acc, dst_reg_tensor, qt_reg_tensor);
        }
            ...
            if constexpr(kDoDQ)
            {
                ...
                if constexpr(kIsFusedFullBwd || !kDoDKDV)
                {
                ...
                else
                {
                    ...
                    if constexpr(kIsDeterministic)
                    {
                        store_tile(dq_dram_window, dq_store_tile);
                    }
                    else
                    {
        ...
        else if constexpr(kDoDKDV)
        {
            ...
            Policy::template PTFromGemm0CToGemm1A<Problem,
                                                  decltype(pt_reg_tensor),
                                                  decltype(p_gemm)>(pt_reg_tensor, p_gemm, ds_lds_ptr);
            auto dot_reg_tensor = load_tile(dot_lds_read_window);
            gemm_1(dv_acc, pt_reg_tensor, dot_reg_tensor);
        }
        HotLoopScheduler::template GemmStagedScheduler<2>();
        ...
        if constexpr(kDoDQ)
        {
            ...
            if constexpr(kIsFusedFullBwd ||
                         (CK_TILE_DEBUG_BWD_SPLIT_DQ_TAIL_DIRECT_UPDATE && !kDoDKDV))
            {
                if constexpr(FmhaDropout::IsDropout)
                {
                    tile_elementwise_inout(
