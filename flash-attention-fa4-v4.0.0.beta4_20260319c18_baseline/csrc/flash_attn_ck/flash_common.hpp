...
inline int get_ck_deterministic_bwd_kn0(int head_size) {
    int device;
    ...
    if (status == hipSuccess) {
        ...
        if (status == hipSuccess) {
            const std::string_view arch_name{props.gcnArchName};
            if (arch_name.starts_with("gfx11") || arch_name.starts_with("gfx12")) {
                return 64;
            }
        }
    }
    return head_size <= 128 ? 128 : 64;
}
