...
def _is_torch_compiling():
    ...
    return bool(dynamo is not None and getattr(dynamo, "is_compiling", lambda: False)())


def _maybe_direct_flash_attn_forward(
    q,
    k,
    v,
    dropout_p,
    ...
    # Debug-only inference shortcut: use tensor requires_grad, not global grad mode,
    # so plain inference tensors can bypass the autograd.Function wrapper.
    if (
        os.getenv("FLASH_ATTN_CK_DEBUG_DIRECT_FWD_INFERENCE", "0") != "1"
        or USE_TRITON_ROCM
        or _is_torch_compiling()
        or dropout_p != 0.0
        or return_attn_probs
        or any(x.requires_grad for x in (q, k, v))
    ):
        return None
    ...
    out_padded, _, _, _ = flash_attn_gpu.fwd(
        q,
        k,
        v,
        None,
...
def flash_attn_qkvpacked_func(
    ...
    """
    direct_out = _maybe_direct_flash_attn_forward(
        qkv[:, :, 0],
        qkv[:, :, 1],
        qkv[:, :, 2],
        dropout_p,
